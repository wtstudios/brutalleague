export const level = await (async () => {
    const j = await fetch("assets/levels/level1/data.json"),
        json = await j.json();
            ...
            const s = p5 => {
                                    ...
                                    void 0,
                                    {
                                        click: function (e) {
                                            if (!e.button) {
                                                ...
                                                document.getElementById('weapon-slot-1').src = levelData.players[playerNum].inventory.activeItem.proto.images.loot;
                                                document.getElementById('weapon-slot-2').src = levelData.players[playerNum].inventory.guns[ absMod((levelData.players[playerNum].inventory.activeIndex - 1), levelData.players[playerNum].inventory.guns.length)].proto.images.loot;
                                                document.getElementById('weapon-name').textContent = levelData.players[playerNum].inventory.activeItem.proto.name;
                                            }
                                        }
                                    }
                                ),
                    ...
                    bullets.forEach((b, i) => {
                        ...
                        if (ob[0] && !gone || b.squaredDistance > b.emitter.ballistics.range ** 2 && !gone || !gone && b.timer >= b.emitter.ballistics.timeout) {
                            if (ob[0]) {
                                ...
                                if (index != -1) {
                                    target.health -= b.emitter.ballistics.damage;
                                    levelData.particles.push(new particle(images.particle1, 255, 10, target.body.position.x + Math.cos(b.angle + Math.PI / 2) * 50 + p5.random(-30, 30), target.body.position.y + Math.sin(b.angle + Math.PI / 2) * 50 + p5.random(-30, 30), b.angle + Math.PI / 2 + p5.random(-1.4, 1.4), target.tint));
                ...
                p5.mousePressed = function () {
                    ...
                    if (p5.mouseButton == p5.LEFT) {
                        sightArray.push({ x: p5.round((levelData.players[playerNum].body.position.x + (p5.mouseX - p5.width / 2) * 2.2) / 10) * 10 - 1100, y: p5.round((levelData.players[playerNum].body.position.y + (p5.mouseY - p5.height / 2) * 2.2) / 10) * 10 - 900});
                    }
                };
                ...
                p5.draw = function () {
                    ...
                    for (let m = 0; m < sightArray.length; m++) {
                        p5.ellipse(sightArray[m].x + 1100, sightArray[m].y + 900, 60, 60);
                        if (m > 0) {
                            p5.line(sightArray[m].x + 1100, sightArray[m].y + 900, sightArray[m - 1].x + 1100, sightArray[m - 1].y + 900);
                        }
                    }
